...
class ModelTester:
    ...
    def cloud_segmentation_test(self, net, test_loader, config, num_votes=10, debug=False): #num_votes=100 
            ...
            if last_min + 1 < new_min:
                ...
                if int(np.ceil(new_min)) % 10 == 0:
                    ...
                    for i, file_path in enumerate(test_loader.dataset.files):
                        ...
                        dot_index = cloud_name.find('.')
                        extracted_text = cloud_name[:dot_index]
                        las_file = join('data', extracted_text + '.las')
                        ...
                        g = lasfile.green
                        b = lasfile.blue
                        las_result = np.column_stack((x, y, z, r, g, b, preds))
                        las_name = join(test_path, 'predictions', extracted_text) 
                        save_las(las_result, las_name)
