...
class S3DISDataset(PointCloudDataset):
    def read_cloud_names_from_file(self, filename):
        try:
            with open(filename, 'r') as file:
                # Read lines from the file and remove leading/trailing whitespaces
                cloud_names = [line.strip() for line in file.readlines()]
            return cloud_names
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True, validation_split=5):
        """
        ...
        self.label_to_names = {0: 'off-ground',
                               1: 'ground'}
        ...
        self.cloud_names = self.read_cloud_names_from_file('data/list.txt') # ['Area_1', 'Area_2', 'Area_3', 'Area_4', 'Area_5']
        self.all_splits = list(range(len(self.cloud_names))) #[0, 1, 2, 3, 4, 5]
        self.validation_split = validation_split # 3
    ...
    def prepare_S3DIS_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            las_file = join(self.path, cloud_name + '.las')
            ...
            object_data = np.column_stack((x, y, z, r, g, b, C))
            xy_min = np.amin(object_data, axis=0)[0:2]
            object_data[:, 0:2] -= xy_min
            ...
            cloud_classes = np.vstack((cloud_classes, object_data[:, 6:7].astype(np.int32)))
...
class S3DISSampler(Sampler):
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False, expected_N=100000):
        """
